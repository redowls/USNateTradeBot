...
log the reason:
- Equity reconciles with the last TRUSTED EOD snapshot in memory/TRADE-LOG.md
  (the most recent EOD snapshot NOT flagged ANOMALY/OPERATOR ALERT; honor any
  later "Operator Note" baseline — never reconcile against a corrupted snapshot
  such as the 2026-06-12 $0 reset): if equity is $0, or deviates >10% beyond
  what open positions' P&L explains,
